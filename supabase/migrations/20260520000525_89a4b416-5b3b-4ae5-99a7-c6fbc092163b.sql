...
BEGIN
  ...
  -- Clear previous reservations for this offer across ALL shipments
  ...
  -- Reset linked_pallets on all responses (re-allocated below)
  ...
  -- Build pending allocations per response (FIFO by created_at)
  ...
  -- Iterate over ALL shipment_items linked to this offer, in shipment creation order
  FOR item IN
  ...
  LOOP
    ...
    -- Track primary (first) shipment for offer fields
    ...
    -- Compute free capacity (excluding our own current reservation, which is 0 after reset)
    ...
    IF item_pallet_capacity <= 0 THEN
      free_capacity := NULL; -- unlimited
    ELSE
      free_capacity := GREATEST(item_pallet_capacity - used_capacity, 0);
    END IF;

    -- FIFO allocate from pending responses to this item
    FOR resp IN
    ...
    LOOP
      ...
      -- Increment linked_pallets on the response (cumulative across shipments)
      UPDATE public.manager_offer_responses
      ...
      -- Ensure distribution exists for (shipment, branch)
      ...
      -- Find or create distribution_item
  ...
  -- Update offer's expected_eta and costs from primary (first) shipment
  ...
  IF NOT any_item_found THEN
    -- No shipment_item linked yet; this can happen briefly before products row created
    RETURN;
  END IF;
END;
$function$;