...
BEGIN
  ...
  -- TRUNCATE avoids the PostgREST "DELETE requires a WHERE clause" safeguard
  -- that aborts unfiltered DELETE statements running in the request session.
  ...
  LOOP
    ...
    FOR resp IN
      SELECT response_id, branch_id, remaining
       ...
       ORDER BY seq
    LOOP
      IF free_capacity IS NOT NULL AND free_capacity <= 0 THEN EXIT; END IF;
      ...
      IF free_capacity IS NOT NULL THEN free_capacity := free_capacity - alloc; END IF;
      ...
      SELECT id INTO dist_id FROM public.distributions
       WHERE shipment_id = item.shipment_id AND branch_id = resp.branch_id LIMIT 1;
      ...
      SELECT di.id, COALESCE(di.pallets, 0) INTO di_id, existing_pallets
        FROM public.distribution_items di
       WHERE di.distribution_id = dist_id AND di.shipment_item_id = item.item_id LIMIT 1;
