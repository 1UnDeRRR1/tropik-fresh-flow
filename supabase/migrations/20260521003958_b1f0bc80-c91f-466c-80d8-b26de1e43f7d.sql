...
DECLARE
  ...
  in_sync text;
BEGIN
  -- Avoid recursion when sync itself updates manager_offers (linked_shipment_id, costs)
  BEGIN
  ...
  IF NEW.status = 'linked' AND NEW.linked_shipment_id IS NOT NULL
    ...
    -- Run FIFO distribution now so linked_pallets reflects what really fits.
    -- Skip if we are already inside a sync (prevents recursion when sync
    -- updates manager_offers.linked_shipment_id / costs).
    IF in_sync IS DISTINCT FROM 'on' THEN
