...
function BranchFreeList() {
  ...
  const totalFree = useMemo(() => rows.reduce((a, r) => a + r.free, 0), [rows]);
  const selectedPallets = pick ? Math.min(Math.max(Number(pallets) || 0, 0), pick.free) : 0;
  const remainingTotal = Math.max(0, totalFree - selectedPallets);
                ...
                <tr className="text-left text-[10px] uppercase tracking-wide text-muted-foreground">
                  ...
                  <th className="px-2 py-2 text-right font-medium">
                    <div className="flex flex-col items-end leading-tight">
                      <span>Палет</span>
                      <span className="text-[11px] font-bold tabular-nums text-destructive">
                        {remainingTotal}
                        {selectedPallets > 0 && (
                          <span className="text-muted-foreground font-medium"> / {totalFree}</span>
                        )}
                        {" п"}
                      </span>
                    </div>
